...
def execute():
    ...
    path_src = os.path.realpath(os.path.join(path_file, "..", "..", "GIS-AWBEM", "src", "GIS-AWBEM")) + "\\"
    path_input = os.path.realpath(os.path.join(path_file, "..", "..", "GIS-AWBEM", "Inputs")) + "\\"
    
    # Define enrichment dictionary
    enrich = {
        "execute_enrichment" : "no", # if yes, providing the following info is required
        "enrichment_file": "",
        "region" : "",         # region: ["north", "south", "east"]
        "mun_growth" : "",     # municipality growth: ["growing", "stagnant"]
        "mun_size" : ""        # municipality size: ["rural", "urban"]
        }
    ...
    config = {
        ...
        "enrich" : enrich,
        "HVAC_type" : "Boiler_DXcoil" # select from ["IdealHVAC", "Boiler_DXcoil", "PTHP"]
        }
